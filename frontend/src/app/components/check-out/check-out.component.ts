...
constructor(private cartService:CartService,private router:Router){
  ...
  this.ingredientItems=this.cartService.getCart().flatMap(i=>i.toppings)
  
   this.pizzaTotal = this.pizzaItems.reduce((sum, item) => sum + (item.pizza.price * item.quantity),0);
  this.ingredientTotal = this.ingredientItems.reduce((sum, t) => sum + t.price,0);
